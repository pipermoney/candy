...
export default async function DigitalAlchemyPage() {
  const allArtworks = await getArtworks()
  const digitalAlchemyArtworks = allArtworks.filter((artwork) => artwork.category === "Digital Alchemy")
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 pt-32 pb-24 max-w-6xl">
        <ScrollReveal>
          <h1 className="text-3xl font-light mb-4">Digital Alchemy</h1>
        ...
        <Suspense fallback={<div className="h-screen flex items-center justify-center">Loading...</div>}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {digitalAlchemyArtworks.map((artwork, index) => (
